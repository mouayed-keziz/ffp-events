...
<div class="mt-8">
    <nav
        class="flex items-center gap-x-2 bg-white px-4 py-2 rounded-md dark:bg-gray-900 dark:ring-white/10 md:px-4 lg:px-4 overflow-x-scroll">
        @if ($sidebar->getTitle() != null || $sidebar->getDescription() != null)
            <div class="me-6 hidden lg:flex flex-col">
                <h3 class="text-base font-medium text-slate-700 bg-red-300 dark:text-white truncate block">
                    {{ $sidebar->getTitle() }}
                </h3>
        ...
        @if (count($sidebar->getNavigationItems()))
            <ul class="flex items-center gap-x-4">
                @foreach ($sidebar->getNavigationItems() as $group)
                    ...
                    @else
                        @foreach ($group->getItems() as $item)
                            <x-filament-page-with-sidebar::topbar.item :active="$item->isActive()" :active-icon="$item->getActiveIcon()"
                                :badge="$item->getBadge()" :badge-color="$item->getBadgeColor()" :icon="$item->getIcon()" :should-open-url-in-new-tab="$item->shouldOpenUrlInNewTab()"
                                :url="$item->getUrl()">
                                {{ $item->getLabel() }}
