...
<div class="form-control">
    ...
    <select class="select select-bordered bg-white mb-2 rounded-md" @if ($data['required'] ?? false) required @endif>
        ...
        @foreach ($data['options'] as $option)
            <option value="{{ $option['option'][app()->getLocale()] }}">{{ $option['option'][app()->getLocale()] }}
            </option>
        @endforeach
    </select>
</div>
