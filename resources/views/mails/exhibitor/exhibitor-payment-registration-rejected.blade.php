...
<head>
    ...
    <title>
        {{ __('emails/exhibitor-payment-registration-rejected.subject', ['event_name' => $event->getTranslation('title', $locale)]) }}
    </title>
</head>
            ...
            <tr style="width:100%">
                <td>
                    ...
                    <p style="color:rgb(0,0,0);font-size:14px;line-height:24px;margin:16px 0">
                        {{ __('emails/exhibitor-payment-registration-rejected.greeting', ['exhibitor_name' => $exhibitor->name]) }}
                    </p>
                    <p style="color:rgb(0,0,0);font-size:14px;line-height:24px;margin:16px 0">
                        {{ __('emails/exhibitor-payment-registration-rejected.intro', ['event_name' => $event->getTranslation('title', $locale)]) }}
                    </p>
                    <p style="color:rgb(0,0,0);font-size:14px;line-height:24px;margin:16px 0">
                        {{ __('emails/exhibitor-payment-registration-rejected.payment_details') }}
                    </p>
                    <ul style="color:rgb(0,0,0);font-size:14px;line-height:24px;margin:16px 0;padding-left:24px;">
                        ...
                        @endphp
                        <li>{{ __('emails/exhibitor-payment-registration-rejected.amount', [
                            'amount' => $paymentSlice->price,
                            'currency' => $paymentSlice->currency,
                        ]) }}
                        </li>
                        <li>{{ __('emails/exhibitor-payment-registration-rejected.date', [
                            'date' => $uploadDate ? $uploadDate->format('d/m/Y') : '-',
                        ]) }}
                        </li>
                        <li>{{ __('emails/exhibitor-payment-registration-rejected.status') }}</li>
                        <li>{{ __('emails/exhibitor-payment-registration-rejected.reason_details', [
                            'rejection_reason' => $submission->rejection_reason,
                        ]) }}
                        </li>
                    </ul>
                    <p style="color:rgb(0,0,0);font-size:14px;line-height:24px;margin:16px 0">
                        {{ __('emails/exhibitor-payment-registration-rejected.instructions') }}
                    </p>
                    <table align="center" width="100%" border="0" cellpadding="0" cellspacing="0"
                                ...
                                <td>
                                    <a href="{{ route('upload_payment_proof', ['slug' => $event->slug]) }}"
                                        ...
                                        target="_blank"><span><!--[if mso]><i style="mso-font-width:500%;mso-text-raise:18" hidden>&#8202;&#8202;</i><![endif]--></span><span
                                            style="max-width:100%;display:inline-block;line-height:120%;mso-padding-alt:0px;mso-text-raise:9px">{{ __('emails/exhibitor-payment-registration-rejected.link_text', ['event_page_link' => route('upload_payment_proof', ['slug' => $event->slug])]) }}</span><span><!--[if mso]><i style="mso-font-width:500%" hidden>&#8202;&#8202;&#8203;</i><![endif]--></span></a>
                                </td>
                            </tr>
                        </tbody>
                    </table>
                    <p style="color:rgb(0,0,0);font-size:14px;line-height:24px;margin:16px 0">
                        {{ __('emails/exhibitor-payment-registration-rejected.assistance') }}
                    </p>
                    <hr
                    ...
                    <p style="color:rgb(102,102,102);font-size:12px;line-height:24px;margin:16px 0">
                        {{ __('emails/exhibitor-payment-registration-rejected.regards') }}<br />
                        {{ __('emails/exhibitor-payment-registration-rejected.team') }}
                    </p>
                </td>
            </tr>
        </tbody>
