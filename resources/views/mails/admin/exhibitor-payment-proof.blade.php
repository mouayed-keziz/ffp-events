...
<body
    ...
    <div style="display:none;overflow:hidden;line-height:1px;opacity:0;max-height:0;max-width:0">
        Preuve de paiement soumise - {{ $event->title }}
    </div>
        ...
        <tbody>
            <tr style="width:100%">
                <td>
                    ...
                    <p style="color:rgb(0,0,0);font-size:14px;line-height:24px;margin:16px 0">
                        Une nouvelle preuve de paiement vient d'être soumise par un exposant pour l'événement
                    ...
                    <p style="color:rgb(0,0,0);font-size:14px;line-height:24px;margin:16px 0">
                        ...
                        Montant: {{ number_format($payment->price, 2) }} {{ $payment->currency }}<br>
                        Date d'échéance: {{ $payment->due_to->format('d/m/Y') }}<br>
                        Événement: {{ $event->title }}
                    </p>
                    <p style="color:rgb(0,0,0);font-size:14px;line-height:24px;margin:16px 0">
                        Veuillez consulter le tableau de bord administrateur pour vérifier cette preuve de paiement et
                        la valider ou la rejeter.
                    </p>
                    <table align="center" width="100%" border="0" cellpadding="0" cellspacing="0"
                            ...
                            <tr>
                                <td>
                                    <a href="{{ route('filament.admin.resources.exhibitor-submissions.view', $submission->id) }}"
                                        style="background-color:#4b5563;border-radius:0.25rem;color:#fff;font-size:12px;font-weight:600;line-height:100%;text-decoration:none;text-align:center;padding-top:12px;padding-bottom:12px;padding-left:24px;padding-right:24px;display:inline-block;max-width:100%"
                                        target="_blank">
                                        Voir la soumission
                                    </a>
                                </td>
